...
function renderIntro() {
  ...
  if (engine.meta.roles) {
    if (!document.getElementById('rc-css')) {
      const s = document.createElement('style');
      s.id = 'rc-css';
      s.textContent = '.rc-title{font-size:14px;font-weight:700;color:var(--text,#e0e6f0);text-align:center;margin-bottom:4px}.rc-sub{font-size:11px;color:var(--muted,#7a8ba8);text-align:center;margin-bottom:10px;line-height:1.4}.rc-row{display:flex;flex-direction:column;gap:8px}.rc-btn{display:flex;flex-direction:column;align-items:flex-start;gap:3px;padding:12px 14px;border:1px solid var(--border,#1e2a45);background:var(--surface,#141b2d);border-radius:10px;cursor:pointer;opacity:.6;transition:all .15s;color:var(--text,#e0e6f0);text-align:left;width:100%}.rc-btn.active{opacity:1;border-color:var(--accent,#3b82f6);box-shadow:0 0 8px rgba(59,130,246,.3)}.rc-head{display:flex;align-items:center;gap:8px}.rc-icon{font-size:20px}.rc-name{font-size:14px;font-weight:700}.rc-tag{font-size:12px;color:var(--muted,#7a8ba8);font-style:italic}.rc-covers{font-size:11px;color:var(--muted,#7a8ba8);opacity:.85}';
      document.head.appendChild(s);
    }
    let host = document.getElementById('intro-role-chooser');
    if (!host) {
      host = document.createElement('div');
      host.id = 'intro-role-chooser';
  ...
  if (engine.revealQueue && engine.revealQueue.length) {
    ...
    queue.forEach((card, i) => {
      setTimeout(() => {
        ...
        if (card.type === 'item') showToast(`📦 ${card.title} added to inventory`, false);
        else showToast(`🟣 ${loreLabel}: ${card.title}`, false);
...
function buildDiscoveryHtml() {
  ...
  active.forEach(d => {
    ...
    if (d.requires_item) {
      ...
      const itemNames = reqs.map(id => engine.cards[id]).filter(c => c && c.type === 'item').map(c => c.title);
      if (itemNames.length) {
        icon = '✅';
        subtitle = `<div style="font-size:11px;color:var(--green);margin-top:2px">✅ Use ${itemNames.join(' + ')}</div>`;
      }
    }
  ...
  locked.forEach(d => {
    ...
    if (d.requires_item) {
      const reqs = Array.isArray(d.requires_item) ? d.requires_item : [d.requires_item];
      const missingNames = reqs
        .filter(r => !(engine.inventory.includes(r) || engine.visibleCards.has(r) || engine.discoveredCards.has(r) || (engine.revealedCards && engine.revealedCards.has(r))))
        .map(id => engine.cards[id]?.title || `#${id}`);
      if (missingNames.length) reqHtml = `<div style="font-size:11px;color:var(--muted);margin-top:3px">🔒 Requires: ${missingNames.join(' + ')}</div>`;
    }
    html += `<button class="discover-btn" style="opacity:.5;cursor:default" disabled><span class="discover-icon">🔒</span><div>${d.label}${reqHtml}</div></button>`;
  });
  html += '</div>';
  return html;
}

...
class BookingRunLock {
  ...
  _render() {
    ...
    const paramStr = Object.entries(c.params).map(([k, v]) => `<span style="color:var(--muted,#7a8ba8)">${k}:</span> <span style="color:#f59e0b">"${v}"</span>`).join(', ');
    ...
    if (this.phase === 'narrate') {
      ...
      this.sceneEl.innerHTML = `<div class="brlk-box brlk-err">${portrait}<div class="brlk-speech brlk-red">${c.fail}</div><div class="brlk-code"><span style="color:var(--red,#ef4444)">❌ ${c.fn}(${c.broken}: <s>"${c.params[c.broken]}"</s> ← FIX)</span></div></div>`;
      ...
      this.sceneEl.innerHTML = `<div class="brlk-box brlk-ok">${portrait}<div class="brlk-speech brlk-green">${c.success}</div><div class="brlk-code" style="color:var(--green,#22c55e)">✅ ${c.fn}(${c.broken}: "${c.answer}")</div></div>`;
...
.brlk-npc{display:flex;align-items:center;gap:8px;margin-bottom:8px;font-size:13px;color:var(--text,#e0e6f0)}
.brlk-portrait{font-size:20px}
.brlk-step{margin-left:auto;font-size:11px;color:var(--muted,#7a8ba8)}
.brlk-box{padding:12px;background:var(--surface,#141b2d);border:1px solid var(--border,#1e2a45);border-radius:8px}
.brlk-box.brlk-err{border-color:var(--red,#ef4444);background:#1a0e0e}
.brlk-box.brlk-ok{border-color:var(--green,#22c55e);background:#0e1a0e}
.brlk-speech{font-size:13px;font-style:italic;color:var(--text,#e0e6f0);margin-bottom:8px}
.brlk-red{color:var(--red,#ef4444)!important}
.brlk-green{color:var(--green,#22c55e)!important}
.brlk-code{font-family:monospace;font-size:12px;padding:8px;background:var(--bg,#0a0e17);border-radius:4px;color:#3b82f6}
.brlk-fix{text-align:center}
.brlk-opts{display:flex;flex-wrap:wrap;gap:6px;justify-content:center}
.brlk-opt{padding:8px 14px;background:var(--surface,#141b2d);border:1px solid var(--border,#1e2a45);border-radius:6px;font-size:12px;font-family:monospace;color:var(--text,#e0e6f0);cursor:pointer;transition:all .15s}
.brlk-opt:active{transform:scale(.95)}
.brlk-btn{padding:10px 20px;background:var(--accent,#3b82f6);color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer}
.brlk-log{font-size:11px;color:var(--muted,#7a8ba8);max-height:60px;overflow-y:auto}
.brlk-log-line{padding:2px 0}
.brlk-done{text-align:center;padding:24px;color:var(--green,#22c55e);font-size:14px}
`;
